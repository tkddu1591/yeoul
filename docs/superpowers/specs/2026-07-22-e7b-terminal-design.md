...
- **배치: 중앙+우측 하단 도크** — 시각 목업 3안(전폭 하단 도크/중앙 분할/분리 창) 중 A(전폭 도크)를 골랐다가, 사용자 수정 제안으로 **좌측 관리 존(변경/실험 공간, E7c 워크트리)은 전체 높이를 유지하고 도크는 중앙(diff)+우측(역사) 아래에만** 여는 v2 목업으로 재확정. 근거: ① 관리 존이 터미널에 밀리지 않음, ② E7c의 "좌측에서 워크트리 선택 → 우측 하단 터미널이 따라감" 좌→우 시선 흐름, ③ 중앙+우측을 세로 래퍼로 묶는 명확한 구현(반응형 computeColumns 무변). 헤더 "터미널" 토글 버튼 + 도크 상단 모서리 드래그 높이 조절. 접으면 0px, **접어도 세션 프로세스는 유지**된다.
...
- `App` 레이아웃: **중앙+우측을 세로 래퍼로 묶고 그 아래에 도크**를 둔다(좌측 열은 래퍼 밖 — 전체 높이 유지). 열림/닫힘·높이(px)는 settings 영속(기존 우측 폭 관례). 기본 높이 ~240px, 최소 120px·최대 창의 60%.
...
| 도크 열림 + 창 축소 | 도크 최대 높이 클램프(창의 60%) — 중앙 diff·우측 역사 min-height 보장(좌측은 무영향) |

...
## E9a 후속 — IME 가드는 E2E로 덮을 수 있었다 (컨트롤러 실측, 2026-07-29)

E9는 이 가드를 **런타임 미검증**으로 남겼다. 구현자와 최종 리뷰어 모두 "Playwright로 네이티브 한글 조합을 주입할 수 없다"고 판단했고 컨트롤러도 그대로 수용했다. **틀렸다.** CDP `Input.imeSetComposition`이 조합 플래그를 세우므로 keydown의 `event.isComposing`이 실제 한글 입력과 똑같이 `true`가 된다.

**실측 경로:** 스테이지 1개 → `Input.imeSetComposition({text:'한'})` → `ControlOrMeta+Enter`. 계측 결과 keydown이 **실제로 도달**했고 `{key:'Enter', metaKey:true, isComposing:true}`였다 — 즉 "커밋 안 됨"이 키 미도달이 아니라 가드의 결과임이 확정된다.

**반증 사이클(2회):** 가드를 `isSubmitEnter(event.key, false)`로 무력화 → `electron-vite build` → 조합 중 커밋이 **실제로 발생**(1→2), 커밋 제목이 **미완성 자모 `한`**. 원복·재빌드 → 차단(1→1), 확정 후 ⌘↵는 정상 커밋.

**중간에 겪은 함정(기록할 가치가 있다):** 첫 반증이 "무력화해도 여전히 차단됨"으로 나와 프로브가 공허해 보였는데, 원인은 **`npx playwright test`가 빌드를 하지 않는다**는 것이었다(`pnpm e2e`만 `electron-vite build &&`를 앞에 붙인다). 무력화한 소스가 앱에 반영되지 않았다. **계측(keydown 기록)을 넣지 않았다면 "가드가 잘 막네"라고 잘못 결론 낼 뻔했다.**

**조치:** `smoke.spec.ts`에 영구 회귀 테스트 1건 추가(키 도달 여부까지 단언해 공허해지지 않게 함). smoke 93 → **94**, e2e 99 → **100**. 단독 2회 비플레이키 확인. 이로써 E9의 "후속 확인 필요" 2건 중 IME 항목은 **폐쇄**된다(남은 1건은 얇은 포커스 링 전역 승격).

...
- [x] **Step 0: 브랜치 생성** — main(4d947b6)에서 `git checkout -b feature/e7d-polish`. `git branch --show-current` 확인.

- [x] **Step 1: Red.** `packages/git-adapter/test/client.test.ts`의 기존(E7c 마지막 worktrees 테스트):
...
- [x] **Step 2: Red 확인** — `pnpm vitest run --project @git-gui/git-adapter -t 'createBranch'` 및 `-t '중복·규칙 위반'` → 옵션 미지원(타입 에러 또는 인자 무시)으로 실패 확인.
...
- [x] **Step 4: Green + 게이트** — `pnpm vitest run --project @git-gui/git-adapter` 전체 통과. 루트 `pnpm test` → **429 passed**. `pnpm typecheck` Done.
...
- [x] **Step 1: 가드 Red.** `apps/desktop/test/worktree-open-guard.test.ts` 신규:
...
- [x] **Step 2: Red 확인 후 구현.** `pnpm vitest run --project @git-gui/desktop -t 'assertOpenableWorktree'` 실패 확인 → `apps/desktop/src/main/worktree-open-guard.ts` 신규:
...
- [x] **Step 3: openPath 핸들러가 순수 가드 사용.** `apps/desktop/src/main/git-handlers.ts` 편집 2곳.
...
- [x] **Step 4: 계약·preload — add에 createBranch.** `packages/ipc-contract/src/index.ts` 기존:
...
- [x] **Step 5: store — addWorktree 시그니처(옵션)·reveal guard 분리.** `apps/desktop/src/renderer/src/store/repository-store.ts` 편집 3곳.
...
- [x] **Step 6: 게이트** — `pnpm typecheck` Done(옵셔널이라 App 무변 컴파일). 루트 `pnpm test` → **432 passed**. `pnpm --filter @git-gui/desktop build` 성공.
...
- [x] **Step 1: 매칭 Red.** `apps/desktop/test/selection-revive.test.ts` 신규:
...
- [x] **Step 2: Red 확인 후 구현.** `pnpm vitest run --project @git-gui/desktop -t 'findRevivableChange'` 실패 확인 → `apps/desktop/src/renderer/src/store/selection-revive.ts` 신규:
...
- [x] **Step 3: store — reviveSelections 헬퍼.** `apps/desktop/src/renderer/src/store/repository-store.ts` 편집 3곳.
...
- [x] **Step 4: 게이트** — `pnpm typecheck` Done. 루트 `pnpm test` → **436 passed**. `pnpm --filter @git-gui/desktop build` 성공.
...
### Task 3-보완: 비교 뷰 닫기 busy 게이트 (품질 리뷰 Important)

**Files:**
- Modify: `apps/desktop/src/renderer/src/components/BranchesPanel.tsx`

branchCompare가 revive 대상이 되면서 생긴 레이스: 갱신(busy) 중 사용자가 비교 뷰를 닫으면(동기 clearBranchCompare) 직후 guard의 revive 쓰기가 닫은 뷰를 되살린다. diff·커밋 상세의 닫기는 이미 busy 게이트(DiffPanel·CommitDetailPanel 관례) — 비교 뷰만 빠져 있었다.
...
(`busy`는 BranchesPanel props에 이미 있다 — grep으로 확인만.)

- [x] **Step 2: 게이트** — `pnpm typecheck` Done. 루트 `pnpm test` → **436 passed**(무변). desktop build 성공.

- [x] **Step 3: Commit**

```bash
git add apps/desktop/src/renderer/src/components/BranchesPanel.tsx
git commit -m "fix(desktop): E7d ⑤ 보완 — 비교 뷰 닫기 busy 게이트(revive가 닫기를 되살리는 레이스)
...
- [x] **Step 1: Red.** `apps/desktop/test/silent-exit.test.ts` 신규:
...
- [x] **Step 2: Red 확인 후 구현.** `pnpm vitest run --project @git-gui/desktop -t 'silentExitNotice'` 실패 확인 → `apps/desktop/src/renderer/src/ui/terminal/silent-exit.ts` 신규:
...
- [x] **Step 3: 훅 배선.** `apps/desktop/src/renderer/src/ui/terminal/use-terminal-sessions.ts` 편집 3곳.
...
- [x] **Step 4: 게이트** — 루트 `pnpm test` → **438 passed**. `pnpm typecheck` Done.
...
### Task 4-보완: 사용자 닫기 오탐 가드 (품질 리뷰 Important)

**Files:**
- Modify: `apps/desktop/src/renderer/src/ui/terminal/use-terminal-sessions.ts`

빠른 닫기 오탐: 무거운 로그인 쉘(.zshrc·nvm)은 첫 프롬프트가 100ms~1s+ 걸린다 — 그 안에 사용자가 탭을 닫으면(close→kill→onExit) 출력 0바이트 종료라 "쉘이 바로 종료됐어요" 오경보가 뜬다. 사용자가 죽인 세션은 판정에서 제외한다.

- [x] **Step 1: 가드 추가.** 편집 2곳.
...
- [x] **Step 2: 게이트** — 루트 `pnpm test` → **438 passed**(무변 — silentExitNotice 순수 판정은 그대로, 호출부만 가드). `pnpm typecheck` Done. desktop build 성공.

- [x] **Step 3: Commit**

```bash
git add apps/desktop/src/renderer/src/ui/terminal/use-terminal-sessions.ts
git commit -m "fix(desktop): E7d ② 보완 — 사용자 닫기(kill) 세션은 깨진 쉘 판정 제외(빠른 닫기 오탐)
...
- [x] **Step 1: Red.** `apps/desktop/test/terminal-theme.test.ts` 신규:
...
- [x] **Step 2: Red 확인 후 구현.** `pnpm vitest run --project @git-gui/desktop -t 'terminalPalette'` 실패 확인 → `apps/desktop/src/renderer/src/ui/terminal/terminal-theme.ts` 신규:
...
- [x] **Step 3: 훅 — theme 인자·생성 팔레트·런타임 교체.** `apps/desktop/src/renderer/src/ui/terminal/use-terminal-sessions.ts` 편집 4곳.
...
- [x] **Step 4: TerminalDock — theme 프롭 관통.** `apps/desktop/src/renderer/src/ui/terminal/TerminalDock.tsx` 편집 3곳.
...
- [x] **Step 5: App — theme 전달.** `apps/desktop/src/renderer/src/App.tsx` 기존:
...
- [x] **Step 6: 게이트** — 루트 `pnpm test` → **440 passed**. `pnpm typecheck` Done. `pnpm --filter @git-gui/desktop build` 성공.
...
### Task 5-보완: 도크 프레임 테마화 (품질 리뷰 Minor — 라이트 심 선제 해소)

**Files:**
- Modify: `apps/desktop/src/renderer/src/ui/terminal/terminal-dock.css`

`.terminal-dock` 배경이 다크 하드코딩(#1a1b23)이라 라이트 모드에서 xterm 라이트 배경(#f7f7fa) 둘레 4px가 어두운 테두리 심으로 노출된다(Task 8 라이트 스크린샷 검수에서 확실히 걸린다 — 선제 해소). 프레임은 앱 서피스 토큰을 따르게 한다.
...
.terminal-dock {
  ...
  border: 1px solid var(--color-border-strong);
  border-radius: 8px;
  /* 프레임은 앱 테마를 따른다 — 터미널 내부 색은 xterm 팔레트(terminal-theme.ts) 소유 (E7d ③ 보완:
     다크 하드코딩이면 라이트 모드에서 xterm 둘레 패딩이 어두운 심으로 노출된다) */
  background: var(--color-surface);
  overflow: hidden;
}
```

- [x] **Step 2: 게이트** — 루트 `pnpm test` → **440 passed**(무변). desktop build 성공. (색 검증은 Task 7 E2E ③과 Task 8 육안 검수가 담당.)

- [x] **Step 3: Commit**

```bash
git add apps/desktop/src/renderer/src/ui/terminal/terminal-dock.css
git commit -m "fix(desktop): E7d ③ 보완 — 도크 프레임을 서피스 토큰으로(라이트 모드 다크 심 해소)
...
- [x] **Step 1: App — 헤더 테마 버튼 제거·changeTheme.** 편집 4곳.
...
- [x] **Step 2: App — ① 충돌 전이 effect.** **(구현 실측 편차 반영 — 최초안은 이 블록을 원위치(이른 반환 뒤)에 두어 Rules of Hooks 위반으로 앱 전체가 깨졌다: repoPath null→값 전이 시 훅 개수가 렌더마다 달라진다. 계산 3줄+effect를 이른 반환 앞으로 이동한다.)**
...
(b) `if (!store.repoPath) {` 이른 반환 **앞**(⌘` 단축키 effect의 닫는 `}, [])` 뒤)에 삽입:
...
(repoPath가 null이면 conflictCount가 항상 0이라 effect는 no-op — 동작 의도 동일.)

- [x] **Step 3: SettingsDialog — [일반|테마] 카테고리.** `apps/desktop/src/renderer/src/ui/settings/SettingsDialog.tsx` **전체 교체**:
...
- [x] **Step 4: AddWorktreeDialog — 모드 라디오.** `apps/desktop/src/renderer/src/components/AddWorktreeDialog.tsx` **전체 교체**:
...
- [x] **Step 6: 게이트** — `pnpm typecheck` Done. 루트 `pnpm test` → **440 passed**(무변). `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **테마 E2E 1건 실패 예상**(theme-toggle 제거) — 그 1건만 실패인지 확인하고 다음 태스크로(Task 7 Step 1이 즉시 재작성한다). 다른 실패가 있으면 원인 수정.
...
### Task 6-보완: 모드 전환 경로 제안 trim 일관화 (품질 리뷰 Minor)

**Files:**
- Modify: `apps/desktop/src/renderer/src/components/AddWorktreeDialog.tsx`

이름 입력 onChange는 trim해서 제안하는데 모드 전환(chooseMode)은 미trim newName을 넘긴다 — 후행 공백이 모드 전환 때만 후행 하이픈 경로가 되는 비일관.
...
- [x] **Step 2: 게이트·커밋** — 루트 `pnpm test` 440 유지·typecheck Done 후:

```bash
git add apps/desktop/src/renderer/src/components/AddWorktreeDialog.tsx
git commit -m "fix(desktop): E7d ④ 보완 — 모드 전환 경로 제안도 trim(onChange 경로와 일관)
...
- [x] **Step 2: 신규 4건.** 파일 끝(E7c 마지막 테스트 `'감시 — 링크드 워크트리를 앱에서 열면...'`의 닫는 `})` 뒤)에 추가:
...
- [x] **Step 3: 게이트** — `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **60 passed**. 신규 4건 각각 단독(-g) 1회 non-flaky 확인. 루트 `pnpm test` → 440, `pnpm typecheck` Done.
...
- [x] **Step 2: README 반영.** `README.md` 기존(E7c 문단 끝):
...
### Task 8-보완: xterm 뷰포트 검은 띠 해소 + 1번 샷 재촬영 (스크린샷 검수)

**Files:**
- Modify: `apps/desktop/src/renderer/src/ui/terminal/terminal-dock.css`

라이트 샷 검수에서 도크 최하단에 검은 띠 발견 — xterm.css 기본 `.xterm .xterm-viewport { background-color: #000 }`(macOS 스크롤바 불투명용)이 스크롤 여백에서 노출된다. 다크에선 팔레트(#1a1b23)와 겹쳐 안 보였고 라이트에서 드러났다(E7b부터 잠복). 특이도 우위 셀렉터로 투명화한다.
...
/* xterm.css 기본 뷰포트 배경(#000 — macOS 스크롤바 불투명용)이 스크롤 여백에서 검은 띠로
   노출된다(라이트에서 발견, 다크에선 팔레트와 겹쳐 잠복 — E7d ③ 스크린샷 검수). 투명화해
   팔레트(.xterm-scrollable-element)·서피스가 그대로 비치게 한다 */
.terminal-dock__view .xterm .xterm-viewport {
  background-color: transparent;
}
```

- [x] **Step 2: 게이트·재촬영** — 루트 `pnpm test` 440 유지·desktop build 후, Task 8 Step 3의 tmp spec 관례로 **1번 샷(e7d-light-terminal-settings)만 재촬영**(다크 띠 소멸 확인). scratchpad 사본 갱신. 생성 후 e2e 재실행 금지.

- [x] **Step 3: Commit**

```bash
git add apps/desktop/src/renderer/src/ui/terminal/terminal-dock.css
git commit -m "fix(desktop): E7d ③ 보완 — xterm 뷰포트 기본 배경(#000) 투명화(라이트 하단 검은 띠)
...
- 커밋 12건: 7b648bb(T1) · c5412c7(T2) · 385ee5b(T3) · 3fb9224(T3 보완: 비교 뷰 닫기 busy 게이트) · cdfcd03(T4) · 48bbfa8(T4 보완: 사용자 닫기 오탐 가드) · 30763a0(T5) · 31b60d5(T5 보완: 도크 프레임 서피스 토큰) · 91e9739(T6) · 142f315(T6 보완: 모드 전환 trim) · d4a8f62(T7) · 3eae184(T8 README) · 792dea2(T8 보완: xterm 뷰포트 검은 띠).
- 최종 게이트(통합 리뷰어 재실측): 단위 **440** · typecheck 전부 Done · E2E **66**(smoke 60 + hosting 6) · last-screen 0건 · 공식 스크린샷 2장 컨트롤러 육안 검수(1번 샷은 검은 띠 발견→보완→재촬영) 통과.
- 구현 편차(전부 본문 레트로 반영): ①의 훅 배치 — 최초안이 이른 반환 뒤라 Rules of Hooks 위반(화이트스크린 실측)→계산 3줄+effect를 이른 반환 앞으로. T7 픽스처 메시지('init')·xterm 배경 실페인트 요소(.xterm-scrollable-element) 실측 보정 2건.
- 품질 리뷰 Important 2건(T3 비교 뷰 닫기 레이스·T4 빠른 닫기 오탐) 보완 폐쇄, Minor 선제 보완 3건(T5 도크 프레임·T6 trim·T8 뷰포트 검은 띠 — 뷰포트 건은 E7b부터 잠복하던 것을 라이트 테마가 드러냄).

## 후속 노트 추가분 (리뷰 이관)

- **대시 시작 브랜치 이름의 원어 에러** — `-b`에서 `-x` 같은 이름은 git이 옵션으로 파싱해 `unknown switch` 원어 노출(생성은 안 됨). `check-ref-format` 선검증 또는 문구 매핑 검토.
- **staging 배치 테스트 부하성 플레이크** — 전체 스위트 첫 실행에서 기존 staging 테스트 3건이 timeout(격리 시 통과, 재실행 클린). E7c 관찰과 동일 클래스 — 픽스처·타임아웃 하드닝 검토.
- **revealWorktree 성공 시 배너 미정리** — guard 분리로 성공해도 이전 에러 배너가 남는다(읽기 전용 동작이라 의도로 수용 — 재확인용 기록).
- **설정 카테고리 상태 유지** — 모달 재열기 시 마지막 카테고리 기억(의도적 수용).

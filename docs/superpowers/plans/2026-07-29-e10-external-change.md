...
**미증명으로 남긴 것(정직 기록):** 신규 3건이 pre-E10 코드에서 실패함을 **리버트로 직접 증명하지는 않았다**(Task 1·2가 이미 들어간 상태였다). 대신 감시 소스를 읽어 세 시나리오가 `.git`에 흔적을 남기지 않음을 코드로 확인했고, 스펙의 독립 실측(0건)과 일치한다. ~~`git checkout -- <file>`이 인덱스를 건드리지 않는다~~ — **이 기록은 틀렸다. 최종 리뷰가 실측으로 반증했다**(아래 I1). 같은 에픽의 스펙 실측표가 이미 `checkout --` → 3건이라고 적고 있었는데도 이 문장이 그와 모순된 채 남았다.
...
## 최종 통합 리뷰 (`main..547a88f`) — 판정 `Fix Important first` → 보완 `9242777`로 폐쇄

리뷰어가 **돌연변이 테스트**로 셋을 증명했다. 게이트는 전부 초록이었고 엔지니어링은 대체로 건전하다는 평가였다.

**I1 — 사용자의 실제 시나리오를 이름에 건 테스트가 실패할 수 없었다.** `watchWorkingTree`를 무력화하고 전체 스위트를 돌리자 **`되돌린 수정` 테스트만 통과**했다(나머지 둘은 실패). 원인: `git checkout -- <file>`은 `.git/index`를 **다시 쓴다**(실측 2건) — 기존 `.git` 감시가 대신 잡는다. 즉 그 테스트는 **pre-E10 코드에서도 통과**하고 워킹트리 감시에 대한 검출력이 0이었다. 그리고 사용자가 실제로 한 **에디터 되돌리기**(순수 워킹트리 쓰기, `.git` 이벤트 0건)는 **테스트가 아예 없었다.** → `writeFile`로 교체. **보완 후 무력화 재현: 세 건 모두 실패**, 원복 후 3/3 통과.

**컨트롤러 기록 오류:** 위 취소선 문장이 그 잘못된 근거였다. **같은 에픽의 스펙이 이미 `checkout --` → 3건이라고 실측해 적어 뒀는데도** 플랜의 다른 문단이 그와 모순된 채 남았고, E2E가 틀린 쪽을 믿고 작성됐다. **한 에픽 안의 두 문서가 어긋나면 실측표가 정본이다.**

**I2 — `armSuppression`이 15곳 중 2곳에만 적용됐다.** Task 2-보완의 논리("읽기 전용 재조회는 쓰지 않으니 억제할 꼬리가 없다")는 13개 액션에 그대로 적용되는데 빠져 있었다(`selectFile`·`selectCommit`·`selectCommitFile`·`compareFileWithWorktree`·`compareBranch`·`viewHistory`·`clearHistoryView`·`selectConflict`·`loadMoreHistory`·`ensureHistoryLoaded`·`revealHead`·`refreshPulls`·`openPullDetail`). **파일을 클릭해 diff를 보는 건 이 앱의 지배적 상호작용**인데 클릭마다 800ms 창이 무장되고, 억제된 이벤트는 **영구 폐기**된다(`:511`, 재시도 없음). `selectFile`은 `status`를 다시 뜨지도 않아 목록이 낡은 채 남는다 — **이 에픽이 없애려던 증상이 더 좁은 창에서 살아남고 있었다.** 13곳 전부 읽기 전용임을 본문으로 확인 후 전환.

**I3 — E10이 새로 만든 회귀: 외부 파일 저장이 화면의 알림을 지운다.** `guard()`가 시작마다 `error: null, notice: null`을 찍는데, pre-E10에는 순수 워킹트리 쓰기가 이벤트를 안 만들어 이 경로가 발화하지 않았다. 이제 **저장할 때마다** 발화한다. 리뷰어가 프로브 + **대조군**으로 확인: 알림 표시 → 억제 창 밖 대기 → 외부 `writeFile` → **4초 안에 알림 사라짐**; 대조군(외부 쓰기 없음)은 5.5초에도 유지. 게다가 `reviveSelections`가 `pullDetail`을 복원하지 않아 **열어 둔 PR 상세 패널도 닫힌다.** 액션이 왜 실패했는지 설명하는 오류 메시지가 **에디터 자동 저장 한 번에 사라진다**(E6b는 알림 수명을 10초로 정했다). → 읽기 전용 재조회는 `error`/`notice`를 지우지 않게, `pullDetail`·`diffLabel`은 `conflictFile`처럼 보존. 회귀 테스트 추가 + 되돌려서 실패 확인.

**M1 — README가 거짓이 됐다:** "E2E 실행 중에는 앱 창이 화면에 나타나지 않습니다" — 포커스 채널 테스트가 `GIT_GUI_E2E_SHOW=1`로 실제 창을 띄우고 `blur()`/`focus()`로 **사용자의 포커스를 뺏는다.** E6a 불변 테스트(`smoke.spec.ts:80`)가 못 잡는 이유는 러너의 env를 읽지 실행별 env를 읽지 않기 때문. 문장 정정.

**M2 — 무시 목록을 두지 않는다는 결정의 근거 숫자가 틀렸다:** 주석의 "209 이벤트 → 재조회 1회"는 실측상 **207 → 2회**다(2초 max-wait이 2.48초짜리 빌드 도중 한 번 발화). 의도적 생략을 떠받치는 문장이라 숫자를 정확히.

**컨트롤러의 또 다른 오귀속 — FSEvents 지연은 800ms가 아니라 12ms다.** 리뷰어 실측 중앙값 **12ms**(8샘플 11~17). 플랜이 "FSEvents 배치 지연"이라 부른 ~800ms는 감시자 지연이 아니라 **종단 간**(감시 12 + 디바운스 300 + 스냅샷 18 + 단언 폴링) 수치였다. Task 2-보완이 필요했던 건 여전히 맞지만 **내가 댄 이유는 틀렸다** — 실제 예산은 약 330ms다. 소스 주석에는 이 오귀속이 없었다(리뷰어 전수 확인).

**리뷰가 검증한 정상 항목:** 디바운스 산술이 **실제 타이머**에서 정확(50ms마다 hit 10초 → 2051·4084·6118·8157·10191ms에 발화 / maxWait 없으면 옛 동작 보존 / max-wait 발화 후 `cycleStartedAt` 누수 없음 / 이벤트 루프 2.6초 블록에도 이중 발화 없음) · **무시 목록 불필요가 측정으로 방어됨**(gitignore 대상에 18,000파일 쓰기 = 18,122 이벤트 → 재조회 **1회**, 이후 `git status` **10ms**; 이 저장소 1.8GB node_modules에서 `fetchSnapshot` 6명령 병렬 **18ms** → 계속 빌드해도 2초당 18ms, 듀티 1% 미만) · `fs.watch(recursive)` 등록 **0.29ms**, RSS +0.5MB, 유휴 이벤트 0 · **피드백 루프 없음**(앱 실측 외부 생성 1회 → 재조회 1회) · 감시 수명주기가 두 감시에도 건전(`stopWatching` 합성·`watchCleanupHooked` 그대로 유효) · 링크드 워크트리 정상 · 실패 모드가 기존과 동일하게 저하 · 억제·포커스 두 테스트 모두 **되돌리면 실패**(공허하지 않음) · 테스트 무결성(삭제·skip 0, 델타가 정확히 신규분).

**남은 잔여(기록):** 저장소 루트 삭제 시 macOS는 `'error'`를 내지 않고 감시가 조용히 멈춘다 — 문서화된 복구 경로와 실제가 다르다(실제 복구는 E10의 포커스 재조회). `isWorkingTreeEvent`는 `/`만 보므로 중첩 `.git`(서브모듈)은 부모 재조회를 유발한다(mac 전용 빌드라 구분자 문제는 무의미).

**최종 게이트(보완 후):** typecheck 6/6 · 루트 **529** · build 성공 · e2e **106**.

...
- [ ] **Step 1: E2E 신규 2건.** smoke.spec.ts 파일 끝(E7h 마지막 테스트 뒤)에 추가. 픽스처·헬퍼는 기존 관례 실독(무인자 `createRepoWithChange()`·`GIT_GUI_E2E_REPO` env 인라인·`hoverAndCmdF` 헬퍼 재사용):
...
- [ ] **Step 2: 게이트** — `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **81 passed**(79+2), 신규 2건 각각 단독 `-g`로 1회 non-flaky, 루트 `pnpm test` 유지, typecheck Done. E2E 포그라운드 동기(timeout 600000).

- [ ] **Step 3: 전체 게이트** — 루트 `pnpm test` **486 내외(478+8 — 실측 정정)** · typecheck 전부 Done · desktop build · `pnpm --filter @git-gui/desktop e2e` → **87**(smoke 81 + hosting 6) · last-screen 아티팩트 0건.

- [ ] **Step 4: README.** 기존 E7h 문단 끝(실독) 뒤에 추가:

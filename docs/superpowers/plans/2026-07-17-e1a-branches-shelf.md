...
- 960px에서 커밋 폼 "저장하기 — N개 파일" 버튼이 두 줄로 꺾인다(보완 실측 중 관찰 — 헤더 범위 밖이라 미수정).
- 보관함 팝오버: 보관하기 클릭 후 ESC 불응(포커스가 body로 떨어짐 — 바깥 클릭으로만 닫힘), "On main:" 접두사 원문 노출(브랜치 배지로 파싱 여지), 스위처 팝오버 기본 스크롤바 스타일, 컨텍스트 메뉴 화살표 키 탐색 부재.
- PromptDialog 에러가 상단 배너에 뜬다(다이얼로그는 유지되나 시선 분리) — 인라인 에러 표시 여지.

- 실험 공간 삭제·이름 바꾸기, 우클릭 메뉴 확장(이 시점으로 되돌리기 revert 등 — 되돌리기 엔진 필요)
- 실험 공간 합치기(merge)·스마트 병합(스펙: 커밋 안 된 변경 자동 보관 후 병합)·충돌 해결 뷰 — 이때 함께: 충돌 파일 변경 취소의 원시 git 에러('unmerged') 친절화, 충돌 diff(`--cc` 마커)의 전용 표시, pop 충돌 상태의 헤더 라벨(MERGE_HEAD 부재로 현재 미표시), 스펙 72행 자동 복원(스마트 체크아웃) 재론

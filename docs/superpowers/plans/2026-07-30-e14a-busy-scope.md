...
- **플랜의 불투명도 단언은 구조적으로 실패할 수 없었다.** 빠른 조회에서 스피너는 정확히 한 프레임만
  ...
  두 단언을 **추가**하고 각각 반증했다.
  **정정(최종 리뷰 Important 3):** 위 문장은 원래 "두 단언으로 **교체**했다"였는데 거짓이었다 —
  검출력 0인 단언이 코드에 그대로 남아 있었다. Task 7에서 실제로 삭제했다.
...
- **테스트 1의 픽스처 크기(선형 200·400·600·800)로는 검출력이 1/3**이었다 → 세제곱(200·1600·5400·12800)
  으로 벌려 **6/9(67%)**. 크기 차가 줄면 검출력이 즉시 떨어지므로, **경합의 정밀한 고정은 단위 테스트의 몫**이고
  이 E2E는 배선 확인용이라는 것을 테스트 주석에 남겼다.
  **정정(최종 리뷰 Important 2):** 여기 적혀 있던 "5/5"는 한 번의 5회 실행만 보고 쓴 수였다.
  리뷰가 `--repeat-each=3`과 `=6`으로 다시 재 **9회 중 6회(67%)**를 실측했다. 측정값으로 바꾼다.

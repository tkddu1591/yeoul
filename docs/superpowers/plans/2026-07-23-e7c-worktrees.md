...
- [ ] **Step 0: 브랜치 생성** — main(6469669)에서 `git checkout -b feature/e7c-worktrees`. `git branch --show-current` 확인.

- [ ] **Step 1: 도메인 타입.** `packages/domain/src/repository.ts` 기존:
...
- [ ] **Step 2: 파서 Red.** `packages/git-adapter/test/worktree-parser.test.ts` 신규:
...
- [ ] **Step 3: Red 확인** — `pnpm vitest run --project @git-gui/git-adapter -t 'parseWorktrees'` → 모듈 부재로 실패 확인.

- [ ] **Step 4: 파서 구현.** `packages/git-adapter/src/worktree-parser.ts` 신규:
...
- [ ] **Step 5: 파서 Green** — 같은 명령 → **6 passed**.

- [ ] **Step 6: client Red.** `packages/git-adapter/test/client.test.ts`의 기존(rebase.abort 테스트 꼬리 — E7b까지의 마지막 rebase 테스트):
...
```ts
  ...
  it('worktrees.list — 본체·링크드·detached·prunable을 담는다', async () => {
    ...
    expect(list[0]).toMatchObject({ path: repo, isMain: true, branch: 'main' })
    expect(list.find((worktree) => worktree.path === `${repo}-feat`)).toMatchObject({
      isMain: false,
      branch: 'feat',
      prunable: false,
    })
    expect(list.find((worktree) => worktree.path === `${repo}-detached`)).toMatchObject({
      branch: null,
    })
    expect(list.find((worktree) => worktree.path === `${repo}-gone`)).toMatchObject({
      prunable: true,
    })
  })
...
- [ ] **Step 7: Red 확인** — `pnpm vitest run --project @git-gui/git-adapter -t 'worktrees'` → **네임스페이스 부재(컴파일/런타임 에러)**로 실패 확인.

- [ ] **Step 8: client 구현.** `packages/git-adapter/src/client.ts` — 편집 4곳.
...
- [ ] **Step 9: Green + 게이트** — `pnpm vitest run --project @git-gui/git-adapter` 전체 통과. 루트 `pnpm test` → **420 passed**. `pnpm typecheck` Done.
...
- [ ] **Step 1: 계약 — repo.openPath.** `packages/ipc-contract/src/index.ts` 기존:
...
- [ ] **Step 3: 핸들러 — 보안 가드 + 5채널.** `apps/desktop/src/main/git-handlers.ts` 편집 3곳.
...
- [ ] **Step 4: preload.** `apps/desktop/src/preload/index.ts` 기존:
...
- [ ] **Step 5: store.** `apps/desktop/src/renderer/src/store/repository-store.ts` 편집 5곳.
...
- [ ] **Step 6: 게이트** — `pnpm typecheck` 전부 Done. 루트 `pnpm test` → **420 passed**(무변). `pnpm --filter @git-gui/desktop build` 성공.
...
- [ ] **Step 1: Red — 필터 테스트 3건.** `apps/desktop/test/watch-filter.test.ts`의 기존(마지막 필터 테스트):
...
- [ ] **Step 2: Red 확인** — `pnpm vitest run --project @git-gui/desktop -t '링크드 워크트리'` → 실패 확인(현행 필터는 worktrees/ 경로를 전부 거부).

- [ ] **Step 3: 필터 구현.** `apps/desktop/src/main/watch-filter.ts` 기존:
...
- [ ] **Step 4: Green** — 필터 테스트 전체(-t 'isRelevantGitEvent' 포함 기존 4건 + 신규 3건) 통과.

- [ ] **Step 5: 감시자 — gitDir 인자화.** `apps/desktop/src/main/repo-watcher.ts` 기존:
...
- [ ] **Step 6: repoWatch 핸들러 — common-dir 해석.** `apps/desktop/src/main/git-handlers.ts` 기존:
...
- [ ] **Step 7: 게이트** — 루트 `pnpm test` → **423 passed**. `pnpm typecheck` Done. `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **51 passed**(감시 E2E 포함 전 스위트 무회귀 — common-dir 전환이 본체 감시를 깨지 않는 게이트).
...
- [ ] **Step 1: 설정 Red.** `packages/ipc-contract/test/settings.test.ts` 기존:
...
- [ ] **Step 2: Red 확인 후 계약 구현.** `pnpm vitest run --project @git-gui/ipc-contract -t '워크트리 선택 동작'` 실패 확인 → `packages/ipc-contract/src/index.ts` 기존:
...
- [ ] **Step 3: load/save 순수.** `apps/desktop/src/renderer/src/ui/settings/worktree-select-action.ts` 신규:
...
- [ ] **Step 4: SettingsDialog.** `apps/desktop/src/renderer/src/ui/settings/SettingsDialog.tsx` 신규:
...
- [ ] **Step 5: CSS.** `apps/desktop/src/renderer/src/ui/settings/settings-dialog.css` 신규:
...
- [ ] **Step 6: App 배선.** `apps/desktop/src/renderer/src/App.tsx` — 편집 3곳.
...
- [ ] **Step 7: 게이트** — 루트 `pnpm test` → **424 passed**. `pnpm typecheck` Done. `pnpm --filter @git-gui/desktop build` 성공.
...
- [ ] **Step 1: 계약.** `packages/ipc-contract/src/index.ts` 기존:
...
- [ ] **Step 2: 핸들러.** `apps/desktop/src/main/terminal-handlers.ts` 기존:
...
- [ ] **Step 4: 훅 — cwd·라벨.** `apps/desktop/src/renderer/src/ui/terminal/use-terminal-sessions.ts` 기존:
...
- [ ] **Step 5: TerminalDock — 활성 워크트리 인자.** `apps/desktop/src/renderer/src/ui/terminal/TerminalDock.tsx` — 편집 3곳(props에 `activeWorktree` 추가, 생성 2곳이 전달).
...
- [ ] **Step 6: App — 임시 null 전달.** Task 6에서 활성 워크트리 상태가 생기기 전까지 컴파일을 지키기 위해, `apps/desktop/src/renderer/src/App.tsx`의 TerminalDock 렌더 기존:
...
- [ ] **Step 7: 게이트** — `pnpm typecheck` Done. 루트 `pnpm test` → **424 passed**(무변). `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **51 passed**(터미널 E2E 4건 무회귀 — cwd 생략 경로).
...
- [ ] **Step 1: 경로 제안 Red.** `apps/desktop/test/worktree-path.test.ts` 신규:
...
- [ ] **Step 2: Red 확인 후 구현.** `pnpm vitest run --project @git-gui/desktop -t 'suggestWorktreePath'` 실패 확인 → `apps/desktop/src/renderer/src/components/worktree-path.ts` 신규:
...
- [ ] **Step 3: WorktreesPanel.** `apps/desktop/src/renderer/src/components/WorktreesPanel.tsx` 신규:
...
- [ ] **Step 4: CSS.** `apps/desktop/src/renderer/src/components/worktrees-panel.css` 신규:
...
- [ ] **Step 5: AddWorktreeDialog.** `apps/desktop/src/renderer/src/components/AddWorktreeDialog.tsx` 신규:
...
- [ ] **Step 6: App 배선 (1) — import.** `apps/desktop/src/renderer/src/App.tsx` 기존:
...
- [ ] **Step 7: App 배선 (2) — leftTab 타입·상태.** 기존:
...
- [ ] **Step 9: App 배선 (4) — 패널 분기.** BranchesPanel 닫힘 뒤의 삼항 종료 기존:
...
- [ ] **Step 10: App 배선 (5) — TerminalDock 활성 전달.** Task 5에서 `activeWorktree={null}`로 둔 줄 기존:
...
- [ ] **Step 11: App 배선 (6) — 다이얼로그 2종.** SettingsDialog 렌더(Task 4) 기존:
...
- [ ] **Step 12: 게이트** — 루트 `pnpm test` → **427 passed**(worktree-path +3). `pnpm typecheck` Done. `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **51 passed**(무회귀).
...
- [ ] **Step 1: E2E 추가.** 파일 끝(마지막 테스트 `'터미널 — 접었다 펴도 세션이 유지된다 (E7b)'`의 닫는 `})` 뒤)에 추가:
...
test('워크트리 탭 — 목록에 본체가 보이고 새로 만든다 (E7c)', async () => {
  ...
  try {
    ...
    await expect(window.getByTestId('add-worktree-path')).toHaveValue(`${repo}-feature-login`)
    await window.getByTestId('add-worktree-submit').click()
    await expect(window.getByTestId('worktree-row-app.txt')).toHaveCount(0)
...
- [ ] **Step 2: 게이트** — `cd apps/desktop && npx electron-vite build && npx playwright test e2e/smoke.spec.ts` → **56 passed**. 신규 5건 각각 단독(-g) 1회 non-flaky 확인. 루트 `pnpm test` → 427, `pnpm typecheck` Done.
...
- [ ] **Step 2: README 반영.** `README.md` 기존(E7b 문단 끝 문장):
...
- [ ] **Step 3: 공식 스크린샷 2장** — `test-results/` + scratchpad(`mkdir -p '/private/tmp/claude-501/-Users-sangyeop-kim-git-gui/b4ef6d32-042d-440c-8252-b8944659aa01/scratchpad'`) 사본. **생성 후 e2e 재실행 금지.** 임시 파일 `apps/desktop/e2e/tmp-shots-e7c.spec.ts`:

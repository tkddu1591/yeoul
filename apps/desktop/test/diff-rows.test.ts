...
import { diffRowsAdapter } from '../src/renderer/src/adapter/diff-rows.adapter'
...
describe('buildDiffRows', () => {
  const hunks: DiffHunk[] = [
    {
      header: '@@ -1,2 +1,2 @@',
      lines: [line('context', 'a'), line('del', 'b'), line('add', 'c')],
    },
    { header: '@@ -10,1 +10,1 @@', lines: [line('context', 'z')] },
  ]

  it('unified — hunk 헤더 행과 라인 행을 순서대로 평탄화한다', () => {
    const rows = diffRowsAdapter.row.toList(hunks, 'unified')
  ...
  it('split — pairHunkLines 결과를 행으로 평탄화한다 (del|add가 한 행)', () => {
    const rows = diffRowsAdapter.row.toList(hunks, 'split')
  ...
  it('빈 hunks면 빈 배열', () => {
    expect(diffRowsAdapter.row.toList([], 'unified')).toEqual([])
  })
})

...
import { diffSplitAdapter } from '../src/renderer/src/adapter/diff-split.adapter'

function line(kind: DiffLine['kind'], text: string): DiffLine {
  return {
    kind,
    oldLine: kind === 'add' || kind === 'note' ? null : 1,
    newLine: kind === 'del' || kind === 'note' ? null : 1,
    text,
  }
}
...
describe('pairHunkLines', () => {
  ...
  it('짝이 남으면 한쪽이 비어 있다', () => {
    const rows = diffSplitAdapter.hunk.pair([line('del', 'a'), line('add', 'A'), line('add', 'B')])
    expect(rows).toEqual([
      { left: line('del', 'a'), right: line('add', 'A') },
  ...
  it('add만 있으면 왼쪽이 비고, del만 있으면 오른쪽이 빈다', () => {
    expect(diffSplitAdapter.hunk.pair([line('add', 'A')])).toEqual([
      { left: null, right: line('add', 'A') },
    ])
    expect(diffSplitAdapter.hunk.pair([line('del', 'a')])).toEqual([
      { left: line('del', 'a'), right: null },
    ])
  })

  it('note는 양쪽에 걸치는 단독 행이다', () => {
    const rows = diffSplitAdapter.hunk.pair([line('note', '\\ No newline at end of file')])
    expect(rows).toEqual([
      {

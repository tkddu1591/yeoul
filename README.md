...
0단계(기반)와 E0·E1(쉬운 모드 — 실험 공간·보관함·합치기)이 동작합니다: 저장소 열기, 상태 감지, 변경 파일 목록, diff 보기(한 줄/좌우 전환, 줄 번호), 체크박스 일괄 stage/unstage/변경 취소, commit, 실험 공간(branch) 전환·만들기(헤더 스위처·우클릭, 막히면 보관함 자동 보관), 보관함(stash) 넣기/꺼내기/버리기, 실험 공간 합치기(merge — 충돌 시 한쪽 고르기/직접 수정/취소, 막히면 자동 보관), 저장된 역사 타임라인(레인 그래프·브랜치/병합 배지, 커밋 클릭 상세, 스크롤로 이어서 불러오기, 열 폭 조절), 백업(push), 저장 메시지 자동 제안 — 디자인 토큰·픽토그램 시스템·3열 레이아웃 적용. 다크/라이트 테마를 전환할 수 있고, 대형 저장소를 위해 변경 목록·역사·diff는 가상 스크롤로 렌더됩니다.
...
1. 최신 받아오기(pull/fetch)·되돌리기(revert)
2. 취소 가능한 Git 프로세스와 실행 로그, 네트워크 진행 표시
3. 충돌 및 중단 상태별 테스트 fixture 확장
